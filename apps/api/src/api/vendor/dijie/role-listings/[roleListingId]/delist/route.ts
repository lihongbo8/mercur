...
function isRoleListingStore(value: unknown): value is DijieRoleListingStore {
  return (
    value !== null &&
    typeof value === "object" &&
    typeof (value as { delistDijieRoleListing?: unknown })
      .delistDijieRoleListing === "function"
  );
}
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  try {
    const result = await store.delistDijieRoleListing({
    ...
    return res.status(400).json({
      ok: false,
      error: error instanceof Error ? error.message : "岗位商品下架失败。",
    });
  }
}
